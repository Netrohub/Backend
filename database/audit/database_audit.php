...
class DatabaseAudit
{
    public function run()
    {
        ...
        // 2. Check for orphaned listings (user deleted but listing exists)
        ...
        // 3. Check for orphaned orders (listing deleted but order exists)
        ...
        if ($duplicateListings->count() > 0) {
            $issues[] = [
                ...
                'details' => $duplicateListings->toArray()
            ];
        }
        ...
        // 12. Check for orders with amount mismatch
        $amountMismatch = DB::table('orders')
            ->join('listings', 'orders.listing_id', '=', 'listings.id')
            ->whereRaw('ABS(orders.amount - listings.price) > 0.01')
            ->count();
        ...
        if ($amountMismatch > 0) {
            $issues[] = [
                ...
                'severity' => 'LOW',
                'message' => 'Found orders where amount does not match listing price',
        ...
        if (empty($issues)) {
            echo "✅ No issues found! Database is clean.\n";
        } else {
            ...
            foreach ($issues as $issue) {
                ...
                echo "{$severityIcon} [{$issue['severity']}] {$issue['type']}\n";
                echo "   {$issue['message']}\n";
                ...
                if (isset($issue['details'])) {
                    echo "   Details: " . json_encode($issue['details'], JSON_PRETTY_PRINT) . "\n";
                }
